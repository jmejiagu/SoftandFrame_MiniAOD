...
// user include files
#include "myAnalyzers/JPsiKsPAT/src/JPsiV0.h"
...
//
JPsiV0::JPsiV0(const edm::ParameterSet& iConfig)
  :
  ...
  Trk1mass_(iConfig.getParameter<double>("Trk1mass")),
  Trk2mass_(iConfig.getParameter<double>("Trk2mass")),
  v0mass_(iConfig.getParameter<double>("v0mass")),
...
void JPsiV0::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using std::vector;
  using namespace edm;
  ...
  for(View<pat::Muon>::const_iterator iMuon1 = thePATMuonHandle->begin(); iMuon1 != thePATMuonHandle->end(); ++iMuon1) 
      ...
      for(View<pat::Muon>::const_iterator iMuon2 = iMuon1+1; iMuon2 != thePATMuonHandle->end(); ++iMuon2) 
	{
	  ...
	  if(J_Prob_tmp<0.01)
	   ...
	   //Now that we have a J/psi candidate, we look for V0Producer candidates
	   if ( theV0PtrHandle->size()>0 && thePATMuonHandle->size()>=2 ) 
	       ...
	       for ( vector<VertexCompositePtrCandidate>::const_iterator iVee = theV0PtrHandle->begin();   iVee != theV0PtrHandle->end(); ++iVee )
		 {
		   ...
		   // ***** In the case of /\b reconstruction, the track with the highest momentum is considered the proton. *********
		   if(v0daughters[0].p() > v0daughters[1].p() ){
		     reco::TransientTrack pion1TT((*theB).build(theDaughterTracks[0]));
		     reco::TransientTrack pion2TT((*theB).build(theDaughterTracks[1]));
		   }
		   else{
		     reco::TransientTrack pion1TT((*theB).build(theDaughterTracks[1]));
		     reco::TransientTrack pion2TT((*theB).build(theDaughterTracks[0]));		     
		   }
		   		   
		   ParticleMass Trak1_mass = Trk1mass_;
		   float Trak1_sigma = Trak1_mass*1.e-6;
		   ParticleMass Trak2_mass = Trk2mass_;
		   float Trak2_sigma = Trak2_mass*1.e-6;
		   ParticleMass V0_mass = v0mass_;
		   float V0_sigma = V0_mass*1.e-6;
		   ...
		   // vector<RefCountedKinematicParticle> muonParticles;
		   try {
		     pionParticles.push_back(pFactory.particle(pion1TT,Trak1_mass,chi,ndf,Trak1_sigma));
		     pionParticles.push_back(pFactory.particle(pion2TT,Trak2_mass,chi,ndf,Trak2_sigma));
		   }
		   catch(...) {
		   ...
		   //if(Ks0_vFit_noMC->currentState().mass()<1.085683 || Ks0_vFit_noMC->currentState().mass()>1.145683) continue;
		   if(Ks0_vFit_noMC->currentState().mass()<TrkTrkMasscut_[0] || Ks0_vFit_noMC->currentState().mass()>TrkTrkMasscut_[1]) continue;
		   ...
		   KinematicConstraint * ks_c = new MassKinematicConstraint(V0_mass,V0_sigma);
		   ...
		   // You can get the momentum components (for muons and kaon) from the final B childrens or of the original Tracks. 
		   // Here, a example for the both tracks:
...
bool JPsiV0::IsTheSame(const reco::Track& tk, const pat::Muon& mu){
